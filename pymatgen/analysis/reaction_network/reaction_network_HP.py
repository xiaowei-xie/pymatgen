...
class ReactionNetwork(MSONable):
    ...
    def solve_prerequisites(self, starts: List[int], weight: str, max_iter=20, save=False,
        ...
        while (len(new_solved_PRs) > 0 or old_attrs != new_attrs) and ii < max_iter:
            ...
            print(ii, len(old_solved_PRs), len(new_solved_PRs), flush=True)
        ...
        for step in path:
            if isinstance(step, int):
                ...
                if step.count("+") == 1:
                    ...
                    print("parse_path something is wrong", path, step, flush=True)
            else:
                assert (("," in step), True)
                    ...
                    for step in paths[node]:
                        if isinstance(step, int):
                            ...
                            if step.count("+") == 1:
                                ...
                                print("SOMETHING IS WRONG", step, flush=True)
                        else:
                            assert(("," in step), True)
                            nodes = nodes+step.split(",")
                    nodes.pop(0)
    ...
    def find_paths(self, starts, target, weight, num_paths=10, solved_PRs_path=False, ignorenode=[]):  # -> ??
        """
