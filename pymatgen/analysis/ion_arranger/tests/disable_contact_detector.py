...
from pymatgen.analysis.ion_arranger.ion_arranger import IonPlacer
...
class TestContactDetector(TestCase):
    def setUp(self):
        ...
        acetoxyq_anion_qcout = QCOutput(
            os.path.join(test_dir, "acetoxyq_anion.out")).data
        pymatgen_acetoxyq = acetoxyq_anion_qcout["initial_molecule"]
        acetoxyq_obmol = BabelMolAdaptor(pymatgen_acetoxyq)._obmol
        tfsi_qcout = QCOutput(os.path.join(test_dir, "tfsi.qcout")).data
        pymatgen_tfsi = tfsi_qcout["initial_molecule"]
        # noinspection PyProtectedMember
    ...
    def test_get_distance_matrix(self):
        ...
        self.assertEqual(1, floydAPSP[0, 1])
        self.assertEqual(2, floydAPSP[0, 2])
        self.assertEqual(3, floydAPSP[0, 3])
        self.assertEqual(2, floydAPSP[0, 4])
        self.assertEqual(1, floydAPSP[1, 2])
        self.assertEqual(2, floydAPSP[1, 3])
        self.assertEqual(1, floydAPSP[1, 4])
        self.assertEqual(3, floydAPSP[2, 3])
        self.assertEqual(2, floydAPSP[2, 4])
        self.assertEqual(1, floydAPSP[3, 4])
